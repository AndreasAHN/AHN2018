...
void stepperTilbage();
   ...
   const int Step1 = 8; //konstant for spole 1
const int Step2 = 9; //konstant for spole 1
const int Step3 = 11; //konstant for spole 2
const int Step4 = 10; //konstant for spole 2
int StepA=1;
int StepB=1;
int i=1;
int h=1;
...
//Intieringer af inputs og outputs
  ...
  const int buzzer = 14;
  const int echoPin = 5; // Ultrasound signal pin
  const int trigPin = 4;
    const int echoPin1 = 7; // Ultrasound signal pin
...
//Mellem hvilken klokkeslet
  if( getTime(1) > 9 &&  getTime(1) < 17)
  {
  digitalWrite(ledOff, LOW);
  digitalWrite(ledOn, HIGH);
    
  while(afstandStandFront() > 25)
    {
    stepperFrem();
