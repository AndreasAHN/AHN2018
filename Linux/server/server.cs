...
namespace tcp
{
	class Communication
	{
		...
		private Socket openConnection(int PORT)
		{
		...
		private bool closeConnection(Socket sock)
		{
			try
			{
				sock.Close();
		...
		private byte[] recive(Socket serverSock)
		{
		...
		private bool transmit(Socket serverSock, byte[] msg)
		{
		...
		private byte[] convertMessageString(string msg)
		{
			byte[] convertet = Encoding.ASCII.GetBytes (msg); 
			return convertet; 
		}

		private byte[] convertMessageInt(int msg)
		{
			byte[] convertet = BitConverter.GetBytes(msg);
			return convertet;
		}

		private string decodeMessageString(byte[] msg)
		{
