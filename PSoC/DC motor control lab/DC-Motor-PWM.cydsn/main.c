/* ========================================
 *
 * Copyright YOUR COMPANY, THE YEAR
 * All Rights Reserved
 * UNPUBLISHED, LICENSED SOFTWARE.
 *
 * CONFIDENTIAL AND PROPRIETARY INFORMATION
 * WHICH IS THE PROPERTY OF your company.
 *
 * ========================================
*/
#include "project.h"
#include <stdio.h>
#include <stdlib.h>
#include <cydevice.h>
CY_ISR_PROTO(ISR_UART_rx_handler);
void handleByteReceived(uint8_t byteReceived);
void decreaseSpeed(void);
void increaseSpeed(void);
void driveForwards(void);
void driveBackwards(void);
void stop(void);
int t = 0;
char f[20];

int main(void)
{
    CyGlobalIntEnable; /* Enable global interrupts. */

    
    isr_uart_rx_StartEx(ISR_UART_rx_handler);
    UART_1_Start();
    PWM_1_Start();
    test_1_Write(0);        //sætter motoren op til at starte emd at køre forlæns
    test_2_Write(1); 
    
    UART_1_PutString("DC-Motor-PWM application started\r\n");
    UART_1_PutString("0: Stop\r\n");
    UART_1_PutString("1: Drive forwards\r\n");
    UART_1_PutString("2: Drive backwards\r\n");
    UART_1_PutString("q: Decrease speed\r\n");
    UART_1_PutString("w: Increase speed\r\n");

    for(;;)
    {
        
    }
}

CY_ISR(ISR_UART_rx_handler)
{
    uint8_t bytesToRead = UART_1_GetRxBufferSize();
    while (bytesToRead > 0)
    {
        uint8_t byteReceived = UART_1_ReadRxData();
        UART_1_WriteTxData(byteReceived); // echo back
        
        handleByteReceived(byteReceived);
        
        bytesToRead--;
    }
}

void handleByteReceived(uint8_t byteReceived)
{
    switch(byteReceived)
    {
        case 'q' :
        {
            decreaseSpeed();
        }
        break;
        case 'w' :
        {
            increaseSpeed();
        }
        break;
        case '1' :
        {
            driveForwards();
        }
        break;
        case '2' :
        {
            driveBackwards();
        }
        break;
        case '0' :
        {
            stop();
            
        }
        break;
        default :
        {
            // nothing
        }
        break;
    }
}

void decreaseSpeed()
{
    UART_1_PutString("Decreasing speed\r\n");
    PWM_1_Start();          
    t = PWM_1_ReadPeriod();                         //gemmer den nuværendene periode
    if(t<=255){                                     //sikre os at perioden ikke overstiger 255
        t++;
        snprintf(f, sizeof(f),"verdi:  %d\r\n", t); 
        UART_1_PutString(f);                        //udskriver den nye periode
    }
    else{}
    PWM_1_WritePeriod(t);                           //sætter den nye periode
}

void increaseSpeed()
{
    UART_1_PutString("Increasing speed\r\n");
    PWM_1_Start();
    t = PWM_1_ReadPeriod();
    if(t>=51){                                      //sikre at perioden ikke kommer under 51
        t--;
        snprintf(f, sizeof(f),"verdi:  %d\r\n", t);
        UART_1_PutString(f);
    }
    else{}
    PWM_1_WritePeriod(t);
}

void driveForwards()
{
    UART_1_PutString("Set direction: forwards\r\n");
    PWM_1_Start();
    //ændre retningen til at køre forlæns
    test_2_Write(0); 
    test_1_Write(1); 
}

void driveBackwards()
{
    UART_1_PutString("Set direction: backwards\r\n");
    PWM_1_Start();
    //ændre retningen til at køre baglæns
    test_1_Write(0); 
    test_2_Write(1); 
}

void stop()
{
    UART_1_PutString("Stop\r\n");
    PWM_1_Stop();
}

/* [] END OF FILE */
