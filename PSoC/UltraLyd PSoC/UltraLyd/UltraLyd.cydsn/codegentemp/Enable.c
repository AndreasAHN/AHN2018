...
{
    uint8 staticBits = (Enable_DR & (uint8)(~Enable_MASK));
    Enable_DR = staticBits | ((uint8)(value << Enable_SHIFT) & Enable_MASK);
}
...
{
	CyPins_SetPinDriveMode(Enable_0, mode);
}
...
{
    return (Enable_PS & Enable_MASK) >> Enable_SHIFT;
}
...
{
    return (Enable_DR & Enable_MASK) >> Enable_SHIFT;
}


/* If interrupt is connected for this Pins component */ 
#if defined(Enable_INTSTAT) 
    ...
    *******************************************************************************/
    ...
    {
		if((position & Enable_0_INTR) != 0u) 
		{ 
			 Enable_0_INTTYPE_REG = (uint8)mode; 
		}
    }
